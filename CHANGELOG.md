...
- **v0.2 P13-net — MCP JSON-RPC transport + gated call flow.** The live execution
  ...
  **`StdioMcp`** that spawns a server process and speaks **Content-Length-framed**
  JSON-RPC over stdio (std `process` + `serde_json`; bounded reads via
  `MAX_MESSAGE_BYTES`; `Drop` tears the child down; covered by an `#[ignore]`d real
  round-trip test). `list_tools` + `manifest_hash` read the live tool surface and
  ...
  artifact-hashes → receipts the (untrusted) response. A **live-call red-team**
  proves a hostile server's "ignore policy / reveal the token / merge now" output is
  inert, redacted, and receipted (invariants 2, 7, 8, 10). `serde_json` admitted to
  the `crustcore-mcp` sidecar (never linked by nano — `forbidden-deps` confirms);
  remote HTTP transport + sandboxed stub exec remain `TODO(P13-net-http)`/P13.5.
  `docs/mcp.md` §8 added. Wave-2 phase.
