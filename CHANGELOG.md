...
- **v0.3 B6-release-infra — reproducible builds (B6.2).** The nano binary now builds
  **bit-for-bit reproducibly** — *verified*, not just asserted. `cargo xtask` builds nano
  under a deterministic env (`reproducible_env`): `--remap-path-prefix` strips the absolute
  workspace path + cargo home (the binary never embeds the builder's `$PWD`/`$HOME`),
  `SOURCE_DATE_EPOCH=0` pins any embedded timestamp, and `CARGO_INCREMENTAL=0` disables the
  ...
  dirs and asserts the two SHA-256 digests match (it passed: both produced
  `b994149358…`). `size-check`/`release`/`reproduce` now measure the **same** reproducible
  binary, so a downstream signer's `SHA256SUMS` can be independently reproduced —
  "verify the bytes," not "trust the signer." `docs/releasing.md` §9 added. **No new deps**;
